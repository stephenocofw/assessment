...
import { LayoutDashboard, ClipboardList, Menu, X, Sun, Moon, Settings as SettingsIcon, TrendingUp, Stethoscope } from 'lucide-react';
...
export function DashboardLayout({ children }: { children: ReactNode }) {
    ...
    // just re-implement standard localStorage logic here for the UI state for now.
    ...
    return (
        <div className="min-h-screen bg-background text-foreground flex relative">
            {/* Mobile Menu Overlay */}
            {isMobileMenuOpen && (
                <div className="fixed inset-0 z-50 bg-background md:hidden flex flex-col p-4 animate-in slide-in-from-right">
                    <div className="flex justify-between items-center mb-8">
                        <Link to="/" className="flex items-center gap-2">
                            <span className="w-8 h-8 rounded-lg bg-primary text-primary-foreground flex items-center justify-center font-bold">FW</span>
                            <span className="text-xl font-bold tracking-tight text-primary">Forge Works</span>
                        </Link>
                    ...
                    <div className="mt-auto border-t pt-4">
                        <button
                            onClick={toggleTheme}
                            className="flex items-center gap-3 px-3 py-3 w-full rounded-md hover:bg-accent hover:text-accent-foreground text-muted-foreground transition-all duration-200"
                        >
            ...
            {/* Sidebar (Desktop) */}
            <aside className="w-64 border-r border-border bg-card p-4 hidden md:flex flex-col justify-between sticky top-0 h-screen">
                <div>
            ...
            {/* Main Content */}
            <main className="flex-1 overflow-auto bg-muted/10 h-screen">
                <header className="md:hidden border-b border-border p-4 flex items-center justify-between bg-card sticky top-0 z-30">
                    <Link to="/">
                        <h1 className="text-lg font-bold flex items-center gap-2">
                            <span className="w-6 h-6 rounded bg-primary text-primary-foreground flex items-center justify-center text-xs">FW</span>
                            Forge Works
                        </h1>
                    </Link>
